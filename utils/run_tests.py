...
if len(files) > 0:  
    if not os.path.exists(os.path.join(parent_path, 'outputs')):
        os.mkdir(os.path.join(parent_path, 'outputs'))

# useful to write output data times
with open(os.path.join(os.path.join(parent_path, 'outputs'), "./metadata.csv"), "w") as text_file: 
    #csv header 
    ...
    for file in files:
        ...
        if file_extension == ".dzn":
            ...
            bashCommand = "python {0}/visualizer/mzn_visualize.py gecode {0}/main.mzn {0}/inputs/{1} {0}/outputs/mzn-gecode{2}.html".format(parent_path,file,input_num)
            ...
            #run first with gecode
            bashCommand = "python {0}/visualizer/mzn_visualize.py coin-bc {0}/main.mzn {0}/inputs/{1} {0}/outputs/mzn-coinbc{2}.html".format(parent_path,file,input_num)
            process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
            output, error = process.communicate()
            out = ast.literal_eval(output)
            if out.has_key("time"):
            ...
            try:
                bashCommand = "python {0}/visualizer/asp_visualize.py {0}/main.lp {0}/inputs/{1} {0}/outputs/asp{2}.html".format(parent_path,file,input_num)
                process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
                output, error = process.communicate()
                if not error:
                    out = ast.literal_eval(output)
